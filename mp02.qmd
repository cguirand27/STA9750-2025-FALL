...
format:
    html:
        theme:  darkly # Change this to any theme below
        code-fold: true
...
America faces a housing affordability crisis, but some cities have found solutions. This project analyzes housing construction, rent burden, and population growth across US metro areas to identify successful pro-housing policies and recommend a federal YIMBY incentive program.
...
### Examining Data set Structure and Identifying Join Keys
...
## Task 7: Policy Brief Development
...
# Extract key statistics for the brief
primary_name <- str_extract(primary_sponsor_city$NAME, "^[^,]+")
cosponsor_name <- str_extract(cosponsor_city$NAME, "^[^,]+")

# Safely extract occupation names
occ1_name <- occupation_1$level2_title
occ2_name <- occupation_2$level2_title  # Remove [1] indexing

# Calculate employment totals directly
occ1_total_employment <- occupation_1$primary_employment + occupation_1$cosponsor_employment
occ2_total_employment <- occupation_2$primary_employment + occupation_2$cosponsor_employment
occ2_avg_share <- mean(c(occupation_2$primary_share, occupation_2$cosponsor_share))

primary_rent_improvement <- abs(round(primary_sponsor_city$rent_burden_change, 0))
cosponsor_rent_burden <- round(cosponsor_city$final_rent_burden, 0)
```

---

<div style="border: 2px solid #2C3E50; padding: 20px; margin: 20px 0; background-color: #ffffff; color: #000000;">

<h2 style="text-align: center; color: #2C3E50; margin-top: 0;">FEDERAL YIMBY INCENTIVE PROGRAM:<br>Building Affordable Communities Through Housing Supply</h2>
<h3 style="text-align: center; color: #495057; margin-top: -10px;">Policy Brief for Congressional Sponsors</h3>
...
**Primary Sponsor: Representative from `r primary_name`**  
This metro exemplifies YIMBY success: despite strong population growth (+`r round(primary_sponsor_city$pop_growth_pct, 0)`%), aggressive housing construction has **reduced rent burden by `r primary_rent_improvement` points** since 2014. Your constituents benefit from both economic growth and improving affordability—a powerful validation of pro-housing policies.

**Co-Sponsor: Representative from `r cosponsor_name`**  
This metro faces a severe affordability crisis: rent burden stands at **`r cosponsor_rent_burden`** (vs. 100 national baseline), among the highest in the nation. Despite population growth, restrictive housing policies have worsened affordability. Your constituents desperately need federal incentives to overcome local NIMBY opposition.

### KEY CONSTITUENCIES

**`r occ1_name` Workers** (`(`r format(occ1_total_employment, big.mark=",")` employees combined)
In high-rent cities, these workers face housing costs `r round(occupation_1$wage_diff_pct, 0)`% higher than in pro-housing metros. Reducing rent burden puts money directly in their pockets—equivalent to a substantial raise without any employer cost.

**`r occ2_name` Sector** (`r format(occ2_total_employment, big.mark=",")` employees, `r round(occ2_avg_share, 1)`% of workforce)
...
Establish competitive federal grants ($500M annually) for municipalities that:
- Streamline permitting (< 6 months for multifamily projects)
- Eliminate single-family-only zoning
- Reduce parking requirements
- Increase allowed density near transit
...
We have proof: `r primary_name` and similar metros show that YIMBY policies improve affordability without sacrificing growth. The alternative—restrictive policies in `r cosponsor_name` and elsewhere—creates housing crises that hurt working families and constrain economic opportunity.
...
<p style="text-align: center; color: #7F8C8D; font-size: 0.9em; margin-bottom: 0;">
For detailed analysis and supporting data, contact [Your Organization]<br>
Based on US Census Bureau data (2009-2023) covering `r n_distinct(yimby_analysis$GEOID)` metropolitan areas
</p>
